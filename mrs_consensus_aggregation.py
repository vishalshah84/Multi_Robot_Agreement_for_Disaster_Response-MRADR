...
MIN_ROBOTS_PER_VICTIM = 3  # must have N_ROBOTS >= N_VICTIMS * this
...
N_VICTIMS   = int(sys.argv[3]) if len(sys.argv) > 3 else 1          # number of true victims
...
victim_discovered = [False] * N_VICTIMS          # has each victim been sensed once?
assignment_done   = False                        # whether robots are assigned to victims
robot_assignments = [-1] * N_ROBOTS              # victim index (0..N_VICTIMS-1) for each robot
...
def build_environment():
    # Create all victims first
    victims = []
    while len(victims) < N_VICTIMS:
        candidate = random_site(is_victim=True)

        # Keep them separated from each other
        ok = True
        for v in victims:
            if np.linalg.norm(candidate.vector - v.vector) < SIGN_MIN_DIST:
                ok = False
                break
        if ok:
            victims.append(candidate)

    # Now create false positives, separated from all victims and from each other
    false_sites = []
    while len(false_sites) < N_FALSE_POS:
        candidate = random_site(is_victim=False)

        ok = True
        # distance from each victim
        for v in victims:
            if np.linalg.norm(candidate.vector - v.vector) < SIGN_MIN_DIST:
                ok = False
                break
        # distance from existing false sites
        if ok:
...
victims, false_sites = build_environment()
victim = victims[0]             # keep this for plots/status (Victim 1)
...
status_text = fig.suptitle("", fontsize=8, fontweight="bold", y=0.98)
...
ax.set_xticks(np.arange(0, WORLD_SIZE + 0.5, 0.5), minor=True)
ax.set_yticks(np.arange(0, WORLD_SIZE + 0.5, 0.5), minor=True)
...
# Draw victims (stars) and false positives (colored squares)
for vidx, v in enumerate(victims):
    ax.plot(v.x, v.y, marker="*", markersize=12,
            color="red", label="Victim" if vidx == 0 else None)
    ax.text(v.x + 0.2, v.y + 0.2, f"Victim {vidx+1}", color="red", fontsize=9)
...
def step_simulation(k):
    ...
    for i, r in enumerate(robots):
        if r.state == "search":
            ...
            # aggregation: go to assigned victim
            if r.assigned_victim is None:
                target = victim.vector  # fallback
            else:
                target = victims[r.assigned_victim].vector
    ...
    for idx, r in enumerate(robots):
        # check all true victims
        for vidx, v in enumerate(victims):
            co2, heat, water, seen = sense_site(r, v)
            if seen and co2 and heat and water:
                r.has_seen_victim = True
                any_seen = True
                victim_discovered[vidx] = True

                # For consensus we still use "victim" (Victim 1)
                # so we anchor beliefs to victim (victims[0])
                r.belief = victim.vector.copy()

                # Record which robot first saw any victim
                if found_robot_index is None:
                    found_robot_index = idx

        # (optional) still sense false positives, even if we don't use it
        for fs in false_sites:
            sense_site(r, fs)
    ...
    # 4) Mode switching: only mark that consensus is reached
    if any(r.has_seen_victim for r in robots) and k > MIN_DETECT_STEPS:
        if not np.any(np.isnan(std)) and np.all(std < CONS_TOL):
            consensus_reached = True


    # 4.5) Once consensus reached AND all victims discovered, assign robots
    global assignment_done, robot_assignments

    if (consensus_reached
        and not assignment_done
        and all(victim_discovered)
        and N_ROBOTS >= N_VICTIMS * MIN_ROBOTS_PER_VICTIM):

        # Distance matrix: dist[i,j] = distance robot i to victim j
        dist = np.zeros((N_ROBOTS, N_VICTIMS))
        for i, r in enumerate(robots):
            for j, v in enumerate(victims):
                dist[i, j] = np.linalg.norm(r.pose_vector() - v.vector)

        robot_assignments = [-1] * N_ROBOTS
        assigned = set()

        # Step 1: ensure at least MIN_ROBOTS_PER_VICTIM per victim
        for j in range(N_VICTIMS):
            # robots sorted by distance to victim j
            order = np.argsort(dist[:, j])
            count = 0
            for i in order:
                if i in assigned:
                    continue
                robot_assignments[i] = j
                assigned.add(i)
                count += 1
                if count >= MIN_ROBOTS_PER_VICTIM:
                    break

        # Step 2: remaining robots go to their closest victim
        for i in range(N_ROBOTS):
            if robot_assignments[i] == -1:
                j_best = int(np.argmin(dist[i, :]))
                robot_assignments[i] = j_best

        # Activate aggregation with these assignments
        for i, r in enumerate(robots):
            r.assigned_victim = robot_assignments[i]
            r.state = "aggregate"
    ...
    for r in robots:
        ...
        d = min(np.linalg.norm(r.pose_vector() - v.vector) for v in victims)
...
def update_anim(frame):
    ...
    if not any(r.has_seen_victim for r in robots):
        ...
        # consensus reached, we are aggregating to multiple victims
        coords = ", ".join(f"V{j+1}:({v.x:.2f},{v.y:.2f})" for j, v in enumerate(victims))
        msg = (f"Consensus reached; splitting robots between {N_VICTIMS} victims "
               f"[{coords}] — Aggregating")
